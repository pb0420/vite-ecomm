...
const DeliverySettingsForm = () => {
  const [settings, setSettings] = useState({ express_fee: '', scheduled_fee: '', late_fee: '' });
  ...
  useEffect(() => {
    const fetchSettings = async () => {
      ...
      try {
        ...
        if (error && error.code !== 'PGRST116') { // Ignore 'No rows found' error if settings don't exist yet
          throw error;
        }
        if (data) {
          setSettings({
            ...
            late_fee: data.late_fee || '', // Include late_fee
          });
        } else {
           // Initialize with defaults if no settings found
           setSettings({ express_fee: '9.99', scheduled_fee: '5.99', late_fee: '7.99' });
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const updateData = {
        id: 1, // Ensure we update the singleton row
      ...
      const { error } = await supabase
        .from('delivery_settings')
        .upsert(updateData, { onConflict: 'id' }); // Use upsert to handle initial creation

      if (error) throw error;

      toast({ title: "Settings Updated", description: "Delivery fees have been saved." });
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-md">
      <div className="space-y-2">
        ...
        <Input
          id="express_fee" name="express_fee" type="number" step="0.01"
          value={settings.express_fee} onChange={handleChange}
      ...
      <div className="space-y-2">
        ...
        <Input
          id="scheduled_fee" name="scheduled_fee" type="number" step="0.01"
          value={settings.scheduled_fee} onChange={handleChange}
       ...
       <div className="space-y-2">
        ...
        <Input
          id="late_fee" name="late_fee" type="number" step="0.01"
          value={settings.late_fee} onChange={handleChange}
          className={errors.late_fee ? 'border-destructive' : ''}
          disabled={isSaving}
        />
        {errors.late_fee && <p className="text-xs text-destructive">{errors.late_fee}</p>}
        <p className="text-xs text-muted-foreground">Fee applied for orders scheduled outside standard hours (example).</p>
      </div>
